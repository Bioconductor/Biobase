...
tkAnnotatedDataFrame <- function(fileName = NULL, sampleNames = NULL,
                                 from = c("file", "object", "edit", "new")) {
    end <- function(){
        tkgrab.release(base)
        tkdestroy(base)
    }
    ...
    getPData <- function(what, desc = NULL){
        switch(what,
               file = {
                   pdata <- 
                       if (!is.null(fileName))
                           importWizard(fileName)[["data"]]
                       else
                           importWizard()[["data"]]
                   if (is.null(pdata)) return(invisible())
                   varList <- list()
                   varList[colnames(pdata)] <- ""},
               object = {
                   pdata <- tkWidgets:::getOBJWidget("data.frame")
                   if (is.null(pdata)) return(invisible())
                   varList <- list()
                   varList[colnames(pdata)] <- ""},
               edit = {
                   pdata <- tkWidgets:::getOBJWidget("AnnotatedDataFrame")
                   if (is.null(pdata)) return(invisible())
                   varList <- varLabels(pdata)
                   pdata <- pData(pdata)},
               new = {
                   sampleNCov <- tkWidgets:::getSNCNums(sampleNames)
                   if (is.null(sampleNCov)) return(invisible())
                   pdata <- data.frame(matrix("",
                                              nrow = sampleNCov$samples,
        ...
        if (!is.null(sampleNames))
            pdata <- tkWidgets:::writePDRowNames(pdata, sampleNames)
        tempData <- tkWidgets:::createPData(pdata, varList)
        if (!is.null(tempData)) {
            annotatedDataFrame <<- tempData
            filename <- tkWidgets:::getName4Data("phenodata", objType = "AnnotatedDataFrame")
            if (!is.null(filename))
                .GlobalEnv[[filename]] <- annotatedDataFrame
        }
    }
    ...
    if (!missing(from)) {
        from <- match.arg(from)
        getPData(from)
        return(invisible(annotatedDataFrame))
    }

    ## no 'from'
    on.exit(end())

    readFile <- function() getPData("file")

    readObject <- function() getPData("object")

    editPheno <- function() getPData("edit")

    createNew <- function() getPData("new")

    actionFrame <- function(parent=base) tkframe(parent, borderwidth=2)

    base <- tktoplevel()
    tktitle(base) <- "Read..."

    tkpack(tklabel(base,
                   text = "AnnotatedDataFrame",
                   font = "Helvetica 16 bold", anchor="w"),
           side = "top", fill="both", padx=5, pady=5)

    actions <- actionFrame()

    newFrame <- actionFrame(actions)
    tkpack(tkbutton(newFrame,
                    text="New...", anchor="w",
                    command=function() getPData("new")))

    createFrame <- actionFrame(actions)
    tkpack(tklabel(actions,
                   text="Create from...", anchor="w"),
           tkbutton(createFrame,
                    text="File", anchor="w",
                    command=function() getPData("file")),
           tkbutton(createFrame,
                    text="Data.frame", anchor="w",
                    command=function() getPData("object")),
           tkbutton(createFrame,
                    text="AnnotatedDataFrame", anchor="w",
                    command=function() getPData("edit")),
           tklabel(createFrame, text="", anchor="w"),
           tkbutton(createFrame,
                    text="New...", anchor="w",
                    command=function() getPData("new")),
           side="top", fill="both", expand=FALSE)

    tkpack(actions, createFrame)

    cancelFrame <- actionFrame()
    tkpack(tkbutton(cancelFrame,
                    text = "Cancel", anchor="e",
                    command = cancel),
           side="right", padx=2)

    tkpack(actions, cancelFrame,
