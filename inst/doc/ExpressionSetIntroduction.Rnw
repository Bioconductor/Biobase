...
%\VignetteIndexEntry{An introduction to Biobase and ExprssionSets}
...
the experiment (\code{featureData}, \code{annotation}), and a flexible
structure to describe the experiment (\code{experimentData}). The
\Rclass{ExpressionSet} class coordinates all of this data, so that you
do not usually have to worry about the details. However, an
\Rclass{ExpressionSet} needs to be created in the first place, and
...
software on your computer, visit \url{http://biocondcutor.org} and
...
\Rpackage{oligo}, \Rpackage{limma}, or \Rpackage{arrayMagic} to read
these files.  These Bioconductor packages have functions (e.g.,
...
initiatives such as GO and KEGG.  The \textit{annotate} package
provides basic data manipulation tools for the meta-data packages.
...
parts, and after all this work the final assembly is disappointingly
easy:
...
information. A minimal expression set can be created with
...
@ 
%% 
Extract the expression matrix and the \Rclass{AnnotatedDataFrame} of
sample information using \Rfunction{exprs} and \Rfunction{phenoData},
respectively:
<<exprs>>=
