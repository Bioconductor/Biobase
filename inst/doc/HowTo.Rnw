...
In most cases HowTo's will be written using \verb+Sweave+ a tool
designed and implemented by F. Leisch. \verb+Sweave+ has been
available in R since R version 1.5.0.

In this document we give some general advice on how to write a HowTo.
Of courese there is no {\em right} way to do this but we feel that we
...
be included in the \verb+% \VignetteIndexEntry+. It should
