...
print(llnames)
@
Now this data can be used to generate a web page.  The {\tt ll.htmlpage}
function of the {\tt annotate} package will accept the
{\tt llnames} vector
and a target filename.
<<>>=
ll.htmlpage(llnames,file="demoLL.html")
@
At this point you can point a browser to {\tt demoLL.html}
to have convenient access to the LL ids and the NCBI
metadata on them.
Don't forget to remove the {\tt demoLL.html} file if
