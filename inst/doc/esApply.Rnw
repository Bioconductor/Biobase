...
print(eset)
print(exprs(eset)[1,])
print(pData(phenoData(eset))[1:2,1:3])
...
print(apply(exprs(eset[1,,drop=F]),1,
  medContr, pData(phenoData(eset))[["cov1"]]))
@
...
medContr2 <- function( y, x ) {
 attach(pData(phenoData(x)))
 on.exit(detach("pData(phenoData(x))"))
 ys <- split(y,cov1)
 median(ys[[1]]) - median(ys[[2]])
 }
