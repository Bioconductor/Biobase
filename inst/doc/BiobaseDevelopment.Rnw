...
extending the \eSet{} class hierarchy, and using features recently
added to \Biobase. The information here is not useful to regular users
of \Biobase.

Recent changes (see the date in the title of this document!) to
\Biobase{} introduce new structure and approaches to \eSet{}. The
changes make it easy for developers to creatively use and extend the
\eSet{} class hierarchy. The purpose of this document is to outline
the changes that have been made, and to illustrate how developers can
exploit the new structure to efficiently use and extend \eSet{}. 

The document starts with a brief description of the motivation for
change, and a comparison of the old and new \eSet{}s and related
functionality (e.g.,the \Rclass{Versioned} class and
\Rfunction{updateObject} methods). We then illustrate how \eSet{} can
be extended to handle additional types of data, and how new methods
can exploit the \eSet{} class hierarchy. We conclude with a brief
summary of lessons learned, useful developer-related side-effects of
efforts to revise \eSet{}, and possible directions for future
development.
...
What is an \eSet{}?
\begin{itemize}
\item Coordinate gene expression and phenotype data.
...
The \eSet{} class structure present in \Biobase{} is:
<<eSet-class>>=
library(Biobase)
getClass("eSet")
@ 
The main functionality of \eSet{} is to coordinate
\Robject{assayData}, \Robject{phenoData}, \Robject{experimentData},
and \Robject{annotation} information, and the structure of \eSet{}
reflects this functionality.  Documentation of the \eSet{} (and of
other classes in \Biobase) can be obtained with the command
\Rfunction{?"eSet-class"}.

Each slot in \eSet{} is defined in terms of a class. We will discuss
\Rclass{AssayData} below; it is used for storing high-throughput data.
\Rclass{AnnotatedDataFrame} coordinates phenotypic data (variables
measured on the samples for which high throughput data were obtained)
and a description of the data. The information in the
\Robject{phenoData} slot is similar to that in previous versions of
\Biobase, but attempts have been made to simplify data storage and
ensure valid object creation (discussed below for \eSet{} as a whole).
\Rclass{MIAME} uses primarily text fields to provide standardized
mechanisms for documenting how the experiment was performed. The
\Robject{annotation} slot is a character string indicating where
\Robject{assayData} features are documented.  \eSet{} can also be
thought of as a generalized container, with high-throughput data
stored in the \Robject{assayData} slot.

\eSet{} is a \verb|VIRTUAL| class. This means that the user cannot
create instances of \eSet{}. Instead, the developer extends \eSet{} in
ways that embody specific functionality. The subclasses
\ExpressionSet{}, \Robject{MultiSet}, and \Robject{SnpSet} are
examples of such extensions.  

There are several benefits to a virtual class like \eSet.
Methods for generic operations can be written once, and are
automatically used by derived classes. Methods defined for \eSet{}
include:
<<eSet-methods>>=
res <- showMethods(where="package:Biobase", classes=c("eSet"), printTo=FALSE)
empty <- which(res=="<Empty Methods List>")
res <- res[-c(empty,empty-1,empty-2)]
res <- res[grep("Function", res)]
res <- unique(gsub('Function \\"(.*)\\":', "\\1", res))
res
@ 
These methods initialize and print \eSet{} (\Rfunction{initialize},
\Rfunction{show}), allow accessing and updating \eSet{} slots (e.g.,
\Rfunction{assayData}, \Rfunction{assayData<-}), and perform
operations common to \eSet{} objects (e.g., \Rfunction{storageMode<-},
to change how data in the \Robject{assayData} slot is stored;
\Rfunction{updateObject} to check and convert, if necessary, a
serialized instance of \eSet{} to the current class definition).

\eSet{} provides consistent structure to data. This is enforced by
validity methods.
<<eSet-validity>>=
...
samples and must be the same in \Robject{assayData} and
...
\subsection{\ExpressionSet: a derived class for expression data}
...
A strength of the \eSet{} hierarchy is apparent from a look at the
methods defined for \ExpressionSet:
<<ExpressionSet-methods>>=
res1 <- showMethods(where="package:Biobase", classes=c("ExpressionSet"), inherited=FALSE, printTo=FALSE)
empty <- which(res1=="<Empty Methods List>")
res1 <- res1[-c(empty,empty-1,empty-2)]
res1 <- res1[grep("Function", res1)]
unique(gsub('Function \\"(.*)\\":', "\\1", res1))
@ 
The designer wrote only a handful of methods to enable the unique
features of the \ExpressionSet{} class; other methods and
functionality are inherited from methods in \eSet{}.

\subsection{\Robject{assayData}: high-throughput data storage}
...
very recently an \R{} object could not \Rfunction{contain} an
\Rclass{environment}.

The \Robject{assayData} slot of \ExpressionSet{} objects must contain
a matrix named \Robject{exprs}. This is different from the structure
of an \Robject{exprSet}, which was expected to contain a slot for
\Robject{exprs} and \Robject{se.exprs} (containing an measure of
uncertain associated with expression measure). The reason for removing
\Robject{se.exprs} from \ExpressionSet is pragmatic: many
\Robject{exprSet} objects did not contain an \Robject{se.exprs}
object. The frequent absence of a data element means that effective
methods cannot be written for the class -- a developer cannot easily
write methods for \Robject{exprSet} that anticipate an
\Robject{se.exprs}, because many objects the method is supposed to
work on may not have the necessary data. Nonetheless, the
\ExpressionSet{} validity method tries to be liberal -- it guarantees
that the object has an \Robject{exprs} element, but allows for other
elements too. The prudent developer wanting consistent additional data
elements should derive a class from \ExpressionSet{} that enforces the
presence of their desired elements.
...
than value. This eliminates some copying, but has the unfortunately
...
environment. Becaue this is very inefficient, the recommended paradigm
...
Developers often want to create a subclass of \eSet, \ExpressionSet,
or other classes in the \eSet{} hierarchy. We will use the
\emph{Swirl} data set in the \Robject{sample.eSet} data set. 

The \emph{Swril} data setcontains four elements in the
...
\Robject{.Object} as a named arguments, or, if there are other named
...
defines the \Rfunction{updateObject} generic and methods for
conveniently updating old objects to their new representation.
<<updateObject-eg>>=
data(sample.ExpressionSet)
...
the other hand, \Rclass{phenoData} contains \Rclass{Versioned}, and
has only information about its own class version.
<<classVersion-phenoData>>=
classVersion(new("phenoData"))
@ 
The rationale for this is that \Rclass{phenoData} is and will likely
...
This document summarizes recent changes to \Biobase, outlining
strategies that developers using \Biobase{} may find useful. The main
points are to introduce the \eSet{} class hierarchy, to illustrate how
...
anticipated that \eSet{}-derived classes will play an increasingly
important role in \Biobase{} development.
